...
import de.bitsnarts.gear.viewer.standardGears.DialogProperties.WheelParams;
...
public class PropertiesDialog extends JPanel implements ActionAndFocusListener {
	...
	class RadProps implements ActionAndFocusListener, DialogPropertiesListener {
		...
		private JTextField field_c;
		private JLabel c_label;
		private JTextField field_xiperc;
		
		public RadProps(WheelParams wheel ) {
			this.wheel = wheel ;
			field_z = new JTextField ( ""+wheel.z ) ;
			addRow ( "Anzahl Zähne", "z", field_z, this ) ;
			field_c = new JTextField ( ""+wheel.c ) ;
			addRow ( "Relative Fußlücke", "c", field_c, c_labeltext (), this ) ;
			c_label = comment_label ;
			field_xiperc = new JTextField ( ""+wheel.xi*100.0 ) ;
			addRow ( "Relative Zahnbreite", "xi", field_xiperc, "%", this ) ;
			props.addDialogPropertiesListener( this );
		}

		private String c_labeltext() {
			return "absolute Fußlücke "+(wheel.c*props.modul)+" mm" ;
		}
		...
		private void handleSource ( Object src ) {
			if ( src == field_z ) {
				...
				c_field_changed () ;
			} else  if ( src == field_xiperc ) {
				field_xiperc_changed () ;
			} else {
				throw new Error ( "unkown source" ) ;
			}			
		}
		
		private void field_xiperc_changed() {
			String txt = field_xiperc.getText() ;
			double val = Double.parseDouble ( txt )/100.0 ;
			if ( val != wheel.xi ) {
				wheel.xi = val ;
				propsChanged () ;
			}
		...
		private void c_field_changed() {
			String txt = field_c.getText() ;
	...
	PropertiesDialog (AbstractDialogProperties props2) {
		this.props = (DialogProperties) props2 ;
		...
		eingr_ang = new JTextField ( ""+props.thetaDegrees ); 
	...
	public static void main ( String args[] ) {
		PropertiesDialog d = new PropertiesDialog ( new DialogProperties() ) ;
	...
	private void eingr_ang_changed() {
		...
		if ( val != props.thetaDegrees ) {
			props.thetaDegrees = val ;
			propsChanged () ;
		}
	}
